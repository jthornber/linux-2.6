			     ...
			     dm_oblock_t oblock, void *unused)
{
	...
	if (ctx->era_match_fn(act_era, ctx->test_era)) {
		DMDEBUG("cblock %u has era %u matching test_era %u; "
			"removing mapping for oblock %llu.",
			from_cblock(cblock), act_era, ctx->test_era,
			oblock);
		      ...
		      struct bio *bio, struct policy_result *result)
{
	...
	/* Check for a mapping */
	r = p->child->map(p->child, oblock, can_block, can_migrate,
			    ...
			    void *hint, bool hint_valid)
{
	...
	r = child->load_mapping(child, oblock, cblock, hint, hint_valid);
			      ...
			      dm_oblock_t new_oblock)
{
	...
	if (!p->child->lookup(p->child, old_oblock, &cblock)) {
