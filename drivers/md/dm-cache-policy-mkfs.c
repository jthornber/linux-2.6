...
static struct entry *hash_lookup(struct mq_policy *mq, dm_oblock_t origin)
{
	unsigned h = hash_64(from_oblock(origin), mq->hash_bits);
...
static void check_generation(struct mq_policy *mq)
{
	...
	if ((mq->hit_count >= mq->generation_period) &&
		...
		mq->promote_threshold = 0; // nr ? total / nr : 1;
 ...
 */
#define DISCARDED_PROMOTE_THRESHOLD 0
#define READ_PROMOTE_THRESHOLD 10000
#define WRITE_PROMOTE_THRESHOLD 40000
...
{
	return dm_cache_policy_register(&mkfs_policy_type);
}

static void __exit mq_exit(void)
{
	dm_cache_policy_unregister(&mkfs_policy_type);
}
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("mkfs cache policy");
