...
void bch_recalc_btree_reserve(struct cache_set *c)
{
	...
	for (i = 0; i < BTREE_ID_NR; i++)
		if (c->btree_roots[i])
			reserve += c->btree_roots[i]->level * 8;
			     ...
			     struct gc_merge_info *r)
{
	...
	pr_debug("coalesced %u -> %u nodes", nodes, nodes - 1);
	gc->nodes--;
