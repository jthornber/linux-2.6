...
static int alloc_entries(struct mq_policy *mq, unsigned elts)
{
	...
	for (u = 0; u < mq->nr_entries; u++) {
		struct mq_entry *e = kmem_cache_alloc(mq_entry_cache, GFP_KERNEL);
 ...
 */
static void hash_insert(struct mq_policy *mq, struct mq_entry *e)
{
	unsigned h = hash_64(from_oblock(e->oblock), mq->hash_bits);
	hlist_add_head(&e->hlist, mq->table + h);
}

static struct mq_entry *hash_lookup(struct mq_policy *mq, dm_oblock_t oblock)
{
...
static void hash_remove(struct mq_entry *e)
{
	hlist_del(&e->hlist);
}
 ...
 */
static struct mq_entry *alloc_entry(struct mq_policy *mq)
{
	struct mq_entry *e;
	...
	e = list_entry(list_pop(&mq->free), struct mq_entry, list);
 ...
 */
static unsigned queue_level(struct mq_entry *e)
{
 ...
 */
static void push(struct mq_policy *mq, struct mq_entry *e)
{
	e->tick = mq->tick;
	hash_insert(mq, e);
 ...
 */
static void del(struct mq_policy *mq, struct mq_entry *e)
{
	queue_remove(&e->list);
	hash_remove(e);
	if (e->in_cache)
 ...
 * recently used).
 */
static struct mq_entry *pop(struct mq_policy *mq, struct queue *q)
{
	struct mq_entry *e = container_of(queue_pop(q), struct mq_entry, list);
 ...
 */
static bool updated_this_tick(struct mq_policy *mq, struct mq_entry *e)
{
 ...
 */
static void requeue_and_update_tick(struct mq_policy *mq, struct mq_entry *e)
{
...
{
	dm_cblock_t result;
	struct mq_entry *demoted = pop(mq, &mq->cache);
...
static void insert_in_pre_cache(struct mq_policy *mq,
				dm_oblock_t oblock)
{
	struct mq_entry *e = alloc_entry(mq);
...
{
	int r = 0;
	struct mq_entry *e = hash_lookup(mq, oblock);
...
{
	struct mq_entry *e = hash_lookup(mq, current_oblock);
...
static int __init mq_init(void)
{
	...
	mq_entry_cache = KMEM_CACHE(mq_entry, 0);
	if (!mq_entry_cache)
		return -ENOMEM;
