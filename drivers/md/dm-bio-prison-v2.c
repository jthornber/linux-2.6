...
static void test_create_destroy(struct dm_bio_prison_v2 *prison,
				struct workqueue_struct *wq)
{
	/* empty */
}

static void test_shared_lock_ordered(struct dm_bio_prison_v2 *prison,
				     struct workqueue_struct *wq)
{
	int r;
	unsigned i;
...
#define COUNT 10000
	static struct dm_bio_prison_cell_v2 *cells[COUNT];

	for (i = 0; i < COUNT; i++) {
		prealloc = dm_bio_prison_alloc_cell_v2(prison, GFP_NOWAIT);
		BUG_ON(!prealloc);
		...
		key.block_begin = i;
		key.block_end = i + 1;
		r = dm_cell_get_v2(prison, &key, 0, bio, prealloc, cells + i);
		BUG_ON(!r);
		BUG_ON(cells[i] != prealloc);
	}

	for (i = 0; i < COUNT; i++) {
		BUG_ON(!dm_cell_put_v2(prison, cells[i]));
		dm_bio_prison_free_cell_v2(prison, cells[i]);
	}
}

static void test_shared_lock_random(struct dm_bio_prison_v2 *prison,
				    struct workqueue_struct *wq)
{
	int r;
	unsigned i;
...
#define COUNT 10000
	...
	for (i = 0; i < COUNT; i++) {
		...
		BUG_ON(!r);
		if (cells[i] != prealloc)
			dm_bio_prison_free_cell_v2(prison, prealloc);
	}

	for (i = 0; i < COUNT; i++)
		if (dm_cell_put_v2(prison, cells[i]))
			dm_bio_prison_free_cell_v2(prison, cells[i]);
}

static void test_shared_repeatedly(struct dm_bio_prison_v2 *prison,
				   struct workqueue_struct *wq)
{
	int r;
	unsigned i;
	...
	key.block_begin = hash_64(i, 32);
	key.block_end = key.block_end + 1;
	r = dm_cell_get_v2(prison, &key, 0, bio, prealloc, &cell);
	BUG_ON(!r);
	...
	for (i = 1; i < count; i++)
		BUG_ON(dm_cell_put_v2(prison, cell));

	BUG_ON(!dm_cell_put_v2(prison, cell));
	dm_bio_prison_free_cell_v2(prison, cell);
}

static void test_shared_at_different_levels(struct dm_bio_prison_v2 *prison,
					    struct workqueue_struct *wq)
{
	int r;
	unsigned i;
	...
	prealloc = dm_bio_prison_alloc_cell_v2(prison, GFP_NOWAIT);
	BUG_ON(!prealloc);
	r = dm_cell_get_v2(prison, &key, 1, bio, prealloc, &cell);
	BUG_ON(!r);
	BUG_ON(cell == prealloc);

	// level 2
	// prealloc wasn't used so, we can use it again.
	r = dm_cell_get_v2(prison, &key, 2, bio, prealloc, &cell);
	BUG_ON(!r);
	BUG_ON(cell == prealloc);
...
static void test_exclusive(struct dm_bio_prison_v2 *prison,
			   struct workqueue_struct *wq)
{
	int r;
	unsigned i;
	struct dm_cell_key_v2 key;
	struct dm_bio_prison_cell_v2 *prealloc;
	struct dm_bio_prison_cell_v2 *excl_cell;
	struct bio_list bios;
	...
	key.block_begin = hash_64(i, 32);
	key.block_end = key.block_end + 1;
	r = dm_cell_lock_v2(prison, &key, 0, prealloc, &excl_cell);
	BUG_ON(r < 0);
	BUG_ON(r);
	BUG_ON(excl_cell != prealloc);

	dm_cell_unlock_v2(prison, excl_cell, &bios);
	BUG_ON(!bio_list_empty(&bios));
	dm_bio_prison_free_cell_v2(prison, excl_cell);
}

static void test_shared_then_exclusive1(struct dm_bio_prison_v2 *prison,
					struct workqueue_struct *wq)
{
	int r;
	unsigned i;
	...
	struct dm_cell_key_v2 key;
	struct dm_bio_prison_cell_v2 *prealloc;
	struct dm_bio_prison_cell_v2 *excl_cell, *shd_cell;
	struct bio_list bios;
	...
	r = dm_cell_get_v2(prison, &key, 0, bio, prealloc, &shd_cell);
	BUG_ON(!r);
	...
	dm_cell_put_v2(prison, shd_cell);
	dm_cell_unlock_v2(prison, excl_cell, &bios);
...
static void drop_cell(struct work_struct *ws)
{
	...
	dm_cell_put_v2(wcell->prison, wcell->cell);
}

static void test_shared_then_exclusive2(struct dm_bio_prison_v2 *prison,
					struct workqueue_struct *wq)
{
	int r;
	unsigned i;
	...
	dm_cell_quiesce_v2(prison, excl_cell, &sem.ws);
...
static void dm_bio_prison_unit_test(void)
{
	static struct {
		const char *name;
		void (*fn)(struct dm_bio_prison_v2 *, struct workqueue_struct *);
	} tests[] = {
	...
	for (i = 0; i < sizeof(tests) / sizeof(*tests); i++) {
		...
		prison = dm_bio_prison_create_v2(wq);
		BUG_ON(!prison);

		tests[i].fn(prison, wq);

		dm_bio_prison_destroy_v2(prison);
