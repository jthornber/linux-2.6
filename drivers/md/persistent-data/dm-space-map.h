...
struct dm_space_map {
	...
	/*
	 * new*_block will increment the returned block.
	 */
	int (*new_block)(struct dm_space_map *sm, dm_block_t *b);
	int (*new_contiguous_blocks)(struct dm_space_map *sm, unsigned count,
...
static inline int dm_sm_new_contiguous_blocks(struct dm_space_map *sm, unsigned count,
					      dm_block_t *b, dm_block_t *e)
{
	return sm->new_contiguous_blocks(sm, count, b, e);
}
